...
> **[DONE/SUPERSEDED 2026-09-09] tickets/ から completed/ へ移動。**
> 元々 `Superseded/Partial`（下記の一部上書き注記の通り）だったが、上書き元の
> `syntax-engine-generic.md`（[[MSE-001]]）自体が完全実装済みになったため、
> 本チケットに残る独自スコープはなくなった。
