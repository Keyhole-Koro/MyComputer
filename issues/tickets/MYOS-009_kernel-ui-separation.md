...
### 対処済み（2026-08-16）：ハードウェアカーソル + 差分更新

実機の作りに倣って両方入れた。実機の GPU が damage tracking を持ちながら別途
ハードウェアカーソルも持つのは、カーソルだけが「毎フレーム動く」特殊な存在
だから。片方だけでは足りない。

**1. ハードウェアカーソル**（MyEmulator `CURSOR_X/Y/CTRL` + `graphics.set_cursor`）

ゲストは座標を書くだけ。スプライトはスキャンアウト時にホストが合成し、VRAM には
書かない。**消すためにシーンを描き直す必要が消える**のが本質で、レジスタという
形式自体に性能上の必然性はない（座標をホストが持つ形でも同じ）。ゲストが表示/
非表示を制御できるようにするため device として実装した。

**2. 差分更新**（`dom.mln` の damage 領域 + `graphics.mln` の clip 矩形）

`X11 Expose` / `Win32 InvalidateRect` と同じ方式。setter が変更ノードの矩形を
記録し、`render_scene` はその領域だけを再描画する。領域に重ならないノードは
`node_in_damage()` で描画自体をスキップする（clip だけでは描画コストを払った
あとにピクセルを捨てるので効果が出ない。実際 clip だけ入れた段階では 1.43M
命令のまま改善しなかった）。

**途中で見つけた罠**: `find_at` はポインタ下の最も深いノードを返すため、ボタン外の
ウィンドウ上ではウィンドウが hover 対象になり 600x400 全体が damage されていた。
しかし `draw_node` を見ると hover で見た目が変わるのは button だけ。
`state_is_visible()` を足して、見た目に影響しない状態変化では damage しない。

**実測**（ボタンへの出入り 116 回 = 再描画 117 回、XTEST で自動化）:
...
ウィンドウ外のポインタ移動では再描画ゼロ。ホスト I/O は実時間の 49% → 約 8%。

**残り**: 473k のうち大半は button 矩形（150x60）の再描画と、コンポジタ 1 パスの
固定費（`find_at`、scheduler tick）。さらに削るなら damage 領域を矩形リストにする
