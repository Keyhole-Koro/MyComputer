...
- **ベクタ格納先**: 割り込みベクタテーブル領域の先頭 `IRQ_VECTOR_ADDR = 0x2400_0100`。
  アーキテクチャ定義で確保された領域なのでRAM/ヒープと完全に分離され衝突しない。既存の
...
### MyEmulator（Rust）
- `constants.rs`: `IRQ_VECTOR_ADDR = 0x2400_0100`、`SR_IE = 0x1`。
...
- トランポリンでのレジスタ退避範囲（r0–r7 全部か、ハンドラが使う分だけか）。最小版は
  全部 push/pop で安全側。
- ベクタ未登録（`0x2400_0100` が 0）のままタイマー割り込みが来た場合のガード
