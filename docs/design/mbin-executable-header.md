...
データセグメントには、コンパイラが `.data` に出すもの（グローバル変数・文字列リテラル・
定数プール）と `.section` で集めた表（アノテーション表）が入る。テキストは命令だけなので
RX で写像でき、W^X が成り立つ（2026-09-21。それまでグローバルはテキストにあり、ユーザ
プロセスのテキストを RWX にする必要があった）。
