...
| セクションは TEXT / DATA の 2 種。実際には `.byte` も TEXT に置かれ、DATA は常に 0 バイト（2026-09-21 に `.data` / `.text` ディレクティブを足し、mlc はグローバル・文字列・定数プールを DATA に出すようになった。ユーザプロセスのテキストを RX にするため） | 「全オブジェクトから同種のものを集める」場所が無い |
